...
package com.android.javacard.FiraSUS;
...
import static com.android.javacard.FiraSUS.Constants.EXP_UNKNOWN_SESSION_ID;
import static com.android.javacard.FiraSUS.Constants.KEY_EXCHANGE_KEY_ID;
import static com.android.javacard.FiraSUS.Constants.MAX_RDS_COUNT;
import static com.android.javacard.FiraSUS.Constants.O_KEY_EXCHANGE_APP_ID;
import static com.android.javacard.FiraSUS.Constants.O_KEY_EXCHANGE_APP_ID_SIZE;
import static com.android.javacard.FiraSUS.Constants.O_KEY_EXCHANGE_ID;
import static com.android.javacard.FiraSUS.Constants.O_OCCUPIED;
import static com.android.javacard.FiraSUS.Constants.O_RDS;
import static com.android.javacard.FiraSUS.Constants.O_RDS_LENGTH;
import static com.android.javacard.FiraSUS.Constants.O_UWB_SESSION_ID;
import static com.android.javacard.FiraSUS.Constants.RANGING_SESSION_KEY;
import static com.android.javacard.FiraSUS.Constants.SEC_PRIVACY_KEY;
import static com.android.javacard.FiraSUS.Constants.STORAGE_RDS_SIZE;
import static com.android.javacard.FiraSUS.Constants.STORE_RDS_PERSISTENT_FLAG;
import static com.android.javacard.FiraSUS.Constants.S_A5;
import static com.android.javacard.FiraSUS.Constants.S_APPLET_OPTIONS;
import static com.android.javacard.FiraSUS.Constants.S_APPLET_OPTIONS_INFO;
import static com.android.javacard.FiraSUS.Constants.S_BFOC;
import static com.android.javacard.FiraSUS.Constants.S_RESPONSE_TEMPLATE;
import static com.android.javacard.FiraSUS.Constants.S_SUS_APPLET_AID;
import static com.android.javacard.FiraSUS.Constants.S_SUS_APP_VERSION;
import static com.android.javacard.FiraSUS.Constants.S_SUS_APP_VERSION_INFO;
import static com.android.javacard.FiraSUS.Constants.UWB_SESSION_ID;
...
public class SUSRDSSlot {
    ...
    public static short createSelectResponse(byte[] reponseBuffer, short reponseBufferOffset) {
        ...
        short offset = 0;
        {
            mBerTlvBuilder.startCOTag(offset);
            {
                ...
                offset = mBerTlvBuilder.addTlv(reponseBuffer,
     ...
     * @return     :- Return UWB Session-Id Offset
     * 
    ...
    private short verifyRangingDataSet(short slotId, byte[] rds, short rdsOffset, short rdsLength) {
        ...
        short ptrOffset = bLinkList.getTLVInstance((short) 1, (short) -1);

        while (ptrOffset != -1) {

            short tagOffset = bLinkList.getTagOffset(ptrOffset);
            ...
            if (rds[tagOffset] == RANGING_SESSION_KEY) {
                ...
                // Storing the session ID here itself to avoid another loop
                retUwbSessionOffset = bLinkList.getValueOffset(ptrOffset);
                shortToByteArray(mRdsInfo, (short) (rdsInfoOffset + O_UWB_SESSION_ID),
                        retUwbSessionOffset);
            } else if (rds[tagOffset] == KEY_EXCHANGE_KEY_ID) {
                // Storing the keyexchangeKey ID here itself to avoid another loop
                shortToByteArray(mRdsInfo, (short) (rdsInfoOffset + O_KEY_EXCHANGE_ID),
    ...
    private static void shortToByteArray(byte[] buffer, short offset, short value) {
        buffer[offset] = (byte) ((value & (short) 0xFF00) >> 8);
        buffer[(short) (offset + 1)] = (byte) (value & 0x00FF);
    }

    private static short readShort(byte[] buffer, short offset) {
        return (short) (((buffer[offset] << 8)) | ((buffer[(short) (offset + 1)] & 0xff)));
    }
    ...
    private boolean compareKeyExchangeKeyId(short slotId, byte[] inBuffer,
        ...
        short keyExchangeKeyIDOffset = (short) (readShort(mRdsInfo,
    ...
    private boolean compareUwbSessionId(short slotId, byte[] inBuffer, short offset) {
        short offsetmRds = (short) (slotId * STORAGE_RDS_SIZE);
        short uwbSessionIdOffset = (short) (readShort(mRdsInfo,
    ...
    private short copyRdsInformation(short slotId, byte[] outBuffer, short outBufferOffset,
        ...
        BerArrayLinkList bLinkList = ((BerTlvParser) mBerTlvParser[slotId]).getBerArrayLinkList();
        short ptrOffsetStart = bLinkList.getTLVInstance((short) 1, (short) -1);
        ...
        while (ptrOffsetStart != -1) {
            ...
            tagOffset = (short) (bLinkList.getTagOffset(ptrOffsetStart) + O_RDS);
            ...
            if (mRdsInfo[tagOffset] >= RANGING_SESSION_KEY
                ...
                if (ptrOffsetEnd != -1) {
                    copiedLength += (Util.arrayCopyNonAtomic(mRdsInfo, tagOffset, outBuffer,
                            destOffset,
                            (short) ((bLinkList.getTagOffset(ptrOffsetEnd) + O_RDS) - tagOffset))
                            - destOffset);
                } else {
    ...
    public void storeRangingDataSet(short slotId, byte[] rds, short rdsOffset, short rdsLength) {
        ...
        shortToByteArray(mRdsInfo, (short) (offsetmRds + O_RDS_LENGTH), rdsLength);
        ...
        shortToByteArray(mRdsInfo, (short) (offsetmRds + O_KEY_EXCHANGE_APP_ID_SIZE), appIdLength);
        ...
        for (short slotId = 0; slotId < MAX_RDS_COUNT; slotId++) {
            ...
            if (mRdsInfoInPersistent[(short) ((slotId * STORAGE_RDS_SIZE) + O_OCCUPIED)] == 0x01) {
                ...
                ((BerTlvParser) mBerTlvParser[slotId]).parse(mRdsInfo,
                        ...
                        readShort(mRdsInfo, (short) (offsetmRds + O_RDS_LENGTH)));
            }
        }
    }
