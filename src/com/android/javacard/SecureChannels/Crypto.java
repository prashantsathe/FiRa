...
public class Crypto {
    ...
    private static byte[] secp256r1_S;
    // Uncompressed form
    private static byte[] secp256r1_UCG;
    private static byte[] secp256r1_N;
    private static final short secp256r1_H = 1;
    ...
    public Crypto() {
        ...
        ZERO = JCSystem.makeTransientByteArray((short) 1, JCSystem.CLEAR_ON_RESET);
    }
    ...
    public boolean verifyECDSAPlainSignatureSha256(
        ...
        try {
          signer = Signature.OneShot.open(MessageDigest.ALG_SHA_256,
              Signature.SIG_CIPHER_ECDSA_PLAIN, Cipher.PAD_NULL);
    ...
    public short ecdSAPlainSignatureSha256(
            byte[] priKey, short priKeyOffset, short priKeyLen,
            byte[] inputDataBuf, short inputDataStart, short inputDataLength,
            byte[] sigBuffer, short sigOffset) {
    ...
    public short generateSecretEC_SVDP_DHC(byte[] priKey, short priKeyOffset, short priKeySize,
                                           byte[] pubKey, short pubKeyOffset, short pubKeySize,
        ...
        return mKeyAgreement.generateSecret(pubKey, pubKeyOffset, pubKeySize,
                secret, secretOffset);
    }

    // X9.63 Key Derivation Function
    public short kdfX963(byte[] shSbuff,short shSOffset, short shSLength,
                         byte[] sharedInfoBuff, short sharedInfoOffset, short sharedInfoLen,
                         short keyDataLen, byte[] out, short outOffset) {
        ...
        // Check that |Z| + |SharedInfo| + 4 < hashmaxlen. If |Z| + |SharedInfo| + 4 ≥ hashmaxlen,
        // output “invalid” and stop.
        // 2. Check that keydatalen < hashlen × (232 − 1). If keydatalen ≥ hashlen × (232 − 1), output
        // “invalid” and stop.

        // Add shared secret 
        Util.arrayCopy(shSbuff, shSOffset, mInputData, (short)0, shSLength);
        // Add counter first 3 bytes (as Key count is 5 )
        mInputData[shSLength] = mInputData[(short)(shSLength + 1)] = mInputData[(short)(shSLength + 2)] = 0x00;
        //Add Shared info
        Util.arrayCopy(sharedInfoBuff, sharedInfoOffset, mInputData, (short) (shSLength + 4), sharedInfoLen);

        try {
            while (index < keyDataLen) {
                mInputData[(short)(shSLength + 3)] = cnt;
                hashLen = mDigest.doFinal(mInputData, (short)0, initialLength, out, (short) (outOffset + index));
    ...
    private static AESKey setAESkey(byte[] key, short keyOffset, short keyLength) {
        ...
        short lenAESkey = keyLength == (short) 16 ? KeyBuilder.LENGTH_AES_128 : KeyBuilder.LENGTH_AES_256;
    ...
    public short genCmacAes128(byte[] key, short keyOffset, short keyLength,
            byte[] inputData, short inputDataOffset, short inputDataLength, byte[] out, short outOffset) {
    ...
    public short genCmacAes128(byte[] key, short keyOffset, short keyLength,
            byte[] inputData, short inputDataOffset, short inputDataLength,
            byte[] chainingValue, short chainingValueOffset, byte[] out, short outOffset) {
...
//    public short genCmacAes256(byte[] key, short keyOffset, short keyLength,
//            byte[] inputData, short inputDataOffset, short inputDataLength, byte[] out, short outOffset) {
//
//        if (mMacSignature256 == null) {
//            mMacSignature256 = Signature.getInstance(Signature.ALG_AES_MAC_256_NOPAD, false);
//        }
//
//        mMacSignature256.init(setAESkey(key, keyOffset, keyLength), Signature.MODE_SIGN);
//        return mMacSignature128.sign(inputData, inputDataOffset, inputDataLength, out, outOffset);
//    }

    public boolean verifyCmacAes128(byte[] key, short keyOffset, short keyLength,
                                  byte[] inputData, short inputDataOffset, short inputDataLength,
                                  byte[] sigBuffer, short sigOffset, short sigLength) {
        ...
        return mMacSignature128.verify(inputData, inputDataOffset, inputDataLength,
                    sigBuffer, sigOffset, sigLength);
    }
    //////////////
    ...
    public short genAes128CbcNopadOutput(byte mode, byte[] keyAes, short keyAesOffset,
                                         byte[] iv, short ivOffset, short ivLength,
                                         byte[] inData, short inDataOffset, short inDataLength,
    ...
    public short genAes128CbcOutput(byte mode, byte[] keyAes, short keyAesOffset,
            byte[] iv, short ivOffset, short ivLength,
            byte[] inData, short inDataOffset, short inDataLength,
            byte[] outData, short outDataOffset) {
    ...
    public short cmacKdfCounterMode(byte[] key, short keyOffset, byte[] label, short labelStart,
                            short labelLen, byte[] L, short Loffset, short Llength, byte counter,
                            byte[] context, short contextStart, short contextLength, byte[] outData,
                            short outDataOffset) {
        ...
        while (i <= counter) {
            ...
            mMacSignature128.update(ZERO, (short) 0, (short) ZERO.length); // 1 byte separation indicator
            mMacSignature128.update(L, Loffset, Llength); // L

            mInputData[0] = i;
            mMacSignature128.update(mInputData, (short) 0, (short) 1); // counter
            mMacSignature128.update(context, contextStart, contextLength); // context

            // signature of 16 bytes
            pos = mMacSignature128.sign(L, (short) Loffset, (short) Llength, outData, (short) (outDataOffset + pos));
            i++;
        }
    ...
    public short cmacKdfCounterModeFiRa2(byte[] key, short keyOffset, byte label1, byte label2,
        ...
        // Note it(input) allows defining a different order than proposed by standard as long as
        ...
        Util.arrayFill(mInputData, inLen, (short) 10, (byte) 0x00);
        inLen += 10;
    ...
    //     o Derivation data = Counter (32b) | Label | Diversification data | Length (32b)
    //     o Derivation data length shall be provisioned to be 128 or 256 bits long
    //     o UWB ranging root key is provisioned in the ADF (see 8.2.2.14.1.2). It shall be either 128 or
    //        256 bits long. Length of derived key will be same as UWB ranging root key
    //     o Label is provisioned in the ADF
    //     o Diversification data is the value taken from the UWB_SESSION_KEY_INFO object
    //        § Diversification data length shall be such that total length or derivation data is 128 or
    //        256 bits as set during provisioning. If Diversification data is too short, 0x0 shall be
    //        inserted to the left to reach diversification data length. If it is too long, only the leftmost
    //        bytes shall be kept.
    //        § For instance, assuming a length of 128 bits for derivation data and a label length of 32
    //        bits, diversification data shall be 128 – 32 (counter) – 32 (label) – 32 (length) = 32
    //        bits, if UWB_SESSION_KEY_INFO is set to 0x67, then diversification data shall be
    ...
    //    o If 256 bits (resp 128 bits) key is provisioned, Length used in derivation shall be 0x00000100
    ...
    public short cmacKdfCounterModeUWBsessionKey(byte[] key, short keyOffset, short keyLength,
            byte[] label, short labelStart, short labelLen, byte[] diversification, short diversificationOffset, short diversificationLength,
            byte[] outData, short outDataOffset) {
        ...
        while (i <= counter) {
            ...
            if (diversificationLength >= (keyLength == (short) 16 ? (short) 4 : (short) 20)) {
                mMacSignature128.update(diversification, diversificationOffset, diversificationLength); // Diversification Data
            } else {
                mMacSignature128.update(mInputData, (short) 8, (short) ((keyLength == (short) 16 ? (short) 4 : (short) 20) - diversificationLength));
                mMacSignature128.update(diversification, diversificationOffset, diversificationLength); // Diversification Data
            }

            // signature of 16 bytes
            pos = mMacSignature128.sign(mInputData, (short) 4, (short) 4, outData, (short) (outDataOffset + pos)); // L
            i++;
        }
