...
public class FiraResponder {
    ...
    public FiraResponder(FiraContext context, FiraClientContext firaClientContext, Crypto crypto, KeyPair ecKeyPair,
            Scp3Lib scp3Lib, Certificates certificates, BerTlvBuilder berTlvBuilder,
            FiraCommon common, byte[] inData, byte[] outData) {
        mContext = context;
    ...
    private short generateMsg2(byte[] outBuff, short outBuffOffset, short outBuffLength, byte[] msg2Enc,
            short msg2EncOffset, short msg2EncLength) {
        ...
        short offset = outBuffOffset;
        {
            ...
            {
                offset = mBerTlvBuilder.addTlv(outBuff, offset, outBuffLength,
    ...
    private short getADF(byte[] buffer, short bufferOffset) {
        return ClientContext.getADFdata(mContext.mContextBuffer, SELECTED_OID_OFFSET,
    ...
    // FiRa Device 2 shall decrypt Msg.1.enc, validate Cert.1 and shall then verify Sig.1. If any of the steps is not
    // successful, FiRa Device 2 shall return error. Note: it will be the responsibility of the Service Provider to
    // provision unambiguously ADF with a public key to check certificate. 
    private short processAsymmetricSC2Authentication(byte[] dataBuff, short dataBuffOffset, short dataBuffLength,
                                                     short cDataOffset, short msg7CByteCnt) {
        ...
        short ShSSize = mCrypto.generateSecretEC_SVDP_DHC(mContext.mContextBuffer, EPHEMERAL_PRIKEY2_OFFSET,
                EC_SK_KEY_LENGTH, mContext.mContextBuffer, EPHEMERAL_PUBKEY1_OFFSET, EC_PK_KEY_LENGTH,
                mInData, (short)0);
        ...
        ShSSize /= 2;
        Util.arrayCopyNonAtomic(mContext.mContextBuffer, (short) (EPHEMERAL_PUBKEY1_OFFSET + 1), mInData,
                ShSSize, LOWER_HIGHER_BYTE_SIZE);

        Util.arrayCopyNonAtomic(mContext.mContextBuffer, (short) (EPHEMERAL_PUBKEY2_OFFSET + 1), mInData,
                (short) (ShSSize + LOWER_HIGHER_BYTE_SIZE), LOWER_HIGHER_BYTE_SIZE);

        // TODO: Key
        kdkLength = mCrypto.genCmacAes128(mContext.mContextBuffer, KEY_PRI_ENC_OFFSET, EC_SK_KEY_LENGTH,
                mInData, (short) 0, (short) (ShSSize + BlOCK_16BYTES), mOutData, (short) 0);

        // third: key expansion (K_SesAuthEnc & KSCP03rootkey(delayed))
        kSesAuthEncLength = mCrypto.cmacKdfCounterModeFiRa2(mOutData, (short) 0, T_B4, T_4B, mOutData, kdkLength);
        ...
        outLen = mCrypto.genAes128CbcNopadOutput(Cipher.MODE_DECRYPT, mOutData, kdkLength,
                mScp3Lib.mNullBytes16, (short) 0, BlOCK_16BYTES, dataBuff,
                (short) (cDataBuffOffset + 2 + msg1EncLengthByteCnt + msg7CByteCnt),
                msg1EncLength, mInData, (short) 0);

        outLen = mCrypto.unpadM2(mInData, (short) 0, outLen);
        ...
        mInData[mesg1ExtLength++] = T_E0;
        mInData[mesg1ExtLength++] = mContext.mContextBuffer[AUTHENTICATE_METHOD_OFFSET];
        Util.arrayCopyNonAtomic(mContext.mContextBuffer, EPHEMERAL_PUBKEY1_OFFSET,
                mInData, mesg1ExtLength, EC_PK_KEY_LENGTH);
        Util.arrayCopyNonAtomic(mContext.mContextBuffer, EPHEMERAL_PUBKEY2_OFFSET,
...
//                        (short) (outLen - 64), (short) 64))
        ...
        if (mContext.mContextBuffer[AUTHENTICATE_METHOD_OFFSET] == ASYM_MUTUAL ||
            ...
            short sig2Len = mCrypto.ecdSAPlainSignatureSha256(mContext.mContextBuffer,
                    EC_KEY_PRIV1_OFFSET, EC_SK_KEY_LENGTH, mInData,(short) 0, msg2extIndex,
                    mInData, msg2plIndex);
            msg2plIndex += sig2Len;
            msg2plIndex += ClientContext.getFiRaCert2(mInData, msg2plIndex);
        } else {
            msg2plIndex += getADF(mInData, msg2plIndex);
        }

        msg2plIndex = (short) (mCrypto.addPaddingM2(mInData, msg2extIndex,
        ...
        short msg2EncLen = mCrypto.genAes128CbcNopadOutput(Cipher.MODE_ENCRYPT, mOutData, kdkLength,
                mScp3Lib.mNullBytes16, (short) 0, SIGNATURE_BLOCK_SIZE, mInData, msg2extIndex,
        ...
        return (short) (generateMsg2(dataBuff, dataBuffOffset, dataBuffLength, mInData,
                msg2plIndex, msg2EncLen) - dataBuffOffset);
    }
    ...
    private short generateGA1ResponseSC1(byte[] outBuff, short outBuffOffset) {
        ...
        short offset = outBuffOffset;
        {
            ...
            RandomData.getInstance(RandomData.ALG_FAST).nextBytes(mContext.mContextBuffer,
                    RANDOM_ICC_OFFSET, BlOCK_16BYTES);
            Util.arrayCopyNonAtomic(mContext.mContextBuffer, RANDOM_ICC_OFFSET, outBuff,
    ...
    private short processGA1ResponseSC1(byte[] dataBuff, short dataBuffOffset, short cDataBuffOffset,
        ...
        if (cDataBuffLength == 4) {
            Util.setShort(mContext.mContextBuffer, SC1_TAGNUMBER_OFFSET, p2);
        } else {
            Util.setShort(mContext.mContextBuffer, SC1_TAGNUMBER_OFFSET,
                    dataBuff[(short) (cDataBuffOffset + 6)]);
        }

        return (short) (generateGA1ResponseSC1(dataBuff, dataBuffOffset) - dataBuffOffset);
    }
    ...
    private short generateGA2ResponseSC1(byte[] outBuff, short outBuffOffset, short outBuffLength,
        ...
        index = Util.arrayCopyNonAtomic(mContext.mContextBuffer, RANDOM_ICC_OFFSET, mInData, index, BlOCK_16BYTES);
        index = Util.arrayCopyNonAtomic(mInData, (short) 0, mInData, index, BlOCK_16BYTES);

        index = RandomData.getInstance(RandomData.ALG_FAST).nextBytes(mInData, index, (short) (BlOCK_16BYTES + 4));
        ...
        short ivLen = mCrypto.genAes128CbcNopadOutput(Cipher.MODE_ENCRYPT, keyBuff,
                macKeyBuffOffset, mScp3Lib.mNullBytes16, (short) 0, BlOCK_16BYTES, mInData, initialOffset,
                BlOCK_16BYTES, mOutData, (short) 0);

        short eICCLen = mCrypto.genAes128CbcNopadOutput(Cipher.MODE_ENCRYPT, keyBuff, encKeyBuffOffset, mOutData,
                (short) 0, ivLen, mInData, initialOffset, (short) outLen, mInData, (short) (outLen + initialOffset));

        short mICCLen = mCrypto.genCmacAes128(keyBuff, macKeyBuffOffset, SIGNATURE_BLOCK_SIZE, mInData,
                (short) (outLen + initialOffset), eICCLen, mInData, (short) (outLen + eICCLen + initialOffset));
        ...
        short offset = outBuffOffset;
        {
            mBerTlvBuilder.startCOTag(offset);
            {
                offset = mBerTlvBuilder.addTlv(outBuff, offset, outBuffLength, RES_82, mInData,
    ...
    private short processGA2ResponseSC1(byte[] dataBuff, short dataBuffOffset, short cDataBuffOffset,
        ...
        short cryptoIFDLen = BerTlvParser.getDataLength(dataBuff, (short) (cDataBuffOffset + 2 + msg7CByteCnt));
        ...
        short outLen = mCrypto.genAes128CbcNopadOutput(Cipher.MODE_ENCRYPT, mInData, macKeyOffset,
                mScp3Lib.mNullBytes16, (short) 0, BlOCK_16BYTES, mContext.mContextBuffer,
                RANDOM_ICC_OFFSET, BlOCK_16BYTES, mOutData, (short) 0);

        outLen = mCrypto.genAes128CbcNopadOutput(Cipher.MODE_DECRYPT, mInData, macKeyOffset, mOutData,
                (short) 0, outLen, dataBuff, (short) (cDataBuffOffset + 2 + msg7CByteCnt + cryptoIFDbyteCnt),
                (short) (cryptoIFDLen - SIGNATURE_BLOCK_SIZE), mInData, (short) 0);

        outLen = Crypto.unpadM2(mInData, (short) 0, outLen);

        if (0 != Util.arrayCompare(mInData, SIGNATURE_BLOCK_SIZE, mContext.mContextBuffer,
                RANDOM_ICC_OFFSET, BlOCK_16BYTES)) {
            ISOException.throwIt(ISO7816.SW_DATA_INVALID);
        }
        ...
        return (short) (generateGA2ResponseSC1(dataBuff, dataBuffOffset, dataBuffLength,
                mInData, encKeyOffset, macKeyOffset) - dataBuffOffset);
    }

    private short processSymmetricSC1Authentication(byte[] dataBuff, short dataBuffOffset, short dataBuffLength,
                                    short cDataOffset, short msg7CByteCnt) {
        ...
        if (dataBuff[(short) (cDataBuffOffset + 2)] == T_81) {
            // set SCP status
            mContext.mContextBuffer[SCP_STATUS_OFFSET] = SC1_GA1;
            return processGA1ResponseSC1(dataBuff, dataBuffOffset, cDataBuffOffset, cDataBuffLength, p2);
        }

        // else it is GA2
        // set SCP status
        mContext.mContextBuffer[SCP_STATUS_OFFSET] = SC1_GA2;
    ...
    private short processGA1ResponseSC2(byte[] dataBuff, short cDataBuffOffset, byte p1) {
        ...
        Util.arrayFill(dataBuff, responseLength, (short) 10, (byte) 0x00); // 10 pre-calculated size
        responseLength += 10;
        ...
        Util.arrayCopyNonAtomic(mOutData, (short) (SIGNATURE_BLOCK_SIZE * 3), mContext.mContextBuffer,
                CRYPTOGRAM2_OFFSET, SIGNATURE_BLOCK_SIZE);

        return (short) (responseLength - cDataBuffOffset);
    }

    private short processSymmetricSC2Authentication(byte[] dataBuff, short dataBuffOffset, short dataBuffLength,
            short cDataOffset, boolean adfPrivacy) {
        ...
        if (dataBuff[(short) (cDataBuffOffset + 2)] == T_81) {
            ...
            short macKeyOffset = ClientContext.getKeyOffet(adfPrivacy ? SC2_PRIVACY_KEYSET : SC2_KEYSET,
            ...
            if (adfPrivacy && cDataBuffLength == (short) 38 &&
                ...
                Util.arrayCopyNonAtomic(mContext.mContextBuffer, RANDOM_DATA0_OFFSET,
                        mInData, (short) 0, BlOCK_16BYTES);
                mInData[16] = 0x00;
                Util.arrayCopyNonAtomic(mContext.mContextBuffer, DEVICE_IDENTIFIER_OFFSET, mInData,
                ...
                if (Util.arrayCompare(dataBuff, (short) (cDataBuffOffset + 22), mOutData,
                    ...
                    // table 40:  If the check fails, a “File or application not found“ error shall be returned.
            ...
            // set SCP status
            mContext.mContextBuffer[SCP_STATUS_OFFSET] = SC2_GA1;
            return processGA1ResponseSC2(dataBuff, dataBuffOffset,p1);
        }

        // (else) 0x82, GA 2
        // verify cryptogram1
        if (Util.arrayCompare(dataBuff, (short) (cDataBuffOffset + 4), mContext.mContextBuffer,
                CRYPTOGRAM2_OFFSET, BlOCK_16BYTES) != 0) {
    ...
    private boolean selectAdf(byte[] oidBuff, short oidBuffOffset, byte oidBuffLength) {
        if (ClientContext.selectAdf(oidBuff, oidBuffOffset, oidBuffLength)) {
            Util.arrayCopyNonAtomic(oidBuff, oidBuffOffset,
                mContext.mContextBuffer, SELECTED_OID_OFFSET, oidBuffLength);
            mContext.mContextBuffer[SELECTED_OID_LENGTH_OFFSET] = oidBuffLength;
            return true;
        }
    ...
    private short generateSelectADFResponse(byte[] outBuff, short outBuffOffset, short outBuffLength,
        ...
        short offset = outBuffOffset;
        {
            ...
            offset = mBerTlvBuilder.addTlv(outBuff, offset, outBuffLength, RES_CD, mInData,
                    (short) 0, (short) 1);

            RandomData.getInstance(RandomData.ALG_FAST).nextBytes(mContext.mContextBuffer,
                    RANDOM_IV_OFFSET, BlOCK_16BYTES);
            ...
            if (tagOrSelKey != 0 && oid != null) {
                ...
                // Privacy Selection Key consists of KPRIV_ENC (used for encryption) and KPRIV_MAC(used for
                // authentication)
                // E = ENC_CBC(KPRIV_ENC, {0x06 ║ L1 ║ OID} ║ {0xCF ║ L2 ║ Diversifier} ║ Padding)

                // First get key set & ENC value;
                short adfLen = mFiraClientContext.getKeySet(tagOrSelKey, mInData, IN_DATA_KEYSET_OFFSET);
                short keyOffset = ClientContext.getKeyOffet(SC1_PRIVACY_KEYSET, ENC_KEYTYPE,
                        mInData, IN_DATA_KEYSET_OFFSET, adfLen);

                short eLength = mCrypto.genAes128CbcNopadOutput(Cipher.MODE_ENCRYPT, mInData, keyOffset,
                        mScp3Lib.mNullBytes16, (short) 0, BlOCK_16BYTES, mInData, (short) 0,
                        index, mOutData, (short) 0);

                // Cryptogram = {0x85 ║ L ║ (RandomData1 ║ RND.IV ║ E)}
                Util.arrayCopyNonAtomic(mContext.mContextBuffer, RANDOM_DATA1_OFFSET, mInData,
                        (short) 0, BlOCK_16BYTES);
                Util.arrayCopyNonAtomic(mContext.mContextBuffer, RANDOM_IV_OFFSET, mInData,
                        BlOCK_16BYTES, BlOCK_16BYTES);
                Util.arrayCopyNonAtomic(mOutData, (short) 0, mInData, (short) (BlOCK_16BYTES * 2), eLength);
 
                offset = mBerTlvBuilder.addTlv(outBuff, offset, outBuffLength, T_85, mInData, (short) 0,
                                            (short) ((BlOCK_16BYTES * 2) + eLength));
                ...
                keyOffset = ClientContext.getKeyOffet(SC1_PRIVACY_KEYSET, MAC_KEYTYPE, mInData,
                        IN_DATA_KEYSET_OFFSET, adfLen);
                short macLength = mCrypto.genCmacAes128(mInData, keyOffset , SIGNATURE_BLOCK_SIZE, mInData,
                        eLengthEnd, (short) (index - eLengthEnd), mOutData, eLength);

                offset = mBerTlvBuilder.addTlv(outBuff, offset, outBuffLength, T_8E, mOutData, eLength,
                        macLength);
            } else {
                // AlgorithmInfo ║ {0x06 ║ Lo ║ OID} ║ {0xCF ║ Ld ║ Diversifier} ║ {0x85 ║ Lr ║ RandomData2}

                offset = mBerTlvBuilder.addTlv(outBuff, offset, outBuffLength, T_06, oid, (short) oidOffset,
                        (short) oidLength);
                offset = mBerTlvBuilder.addTlv(outBuff, offset, outBuffLength, T_CF, mContext.mContextBuffer,
                        DEVICE_IDENTIFIER_OFFSET, DEVICE_IDENTIFIER_SIZE);
                offset = mBerTlvBuilder.addTlv(outBuff, offset, outBuffLength, T_85, mContext.mContextBuffer,
                        RANDOM_DATA1_OFFSET, BlOCK_16BYTES);
            }
        }
        return offset;
    }
    ...
    private short parseSelectAdfSC1(byte[] dataBuff, short dataOffset, short dataLength,
        ...
        while (index < end) {
            ...
            if (dataBuff[index] == T_83) {
                ...
                Util.arrayCopyNonAtomic(dataBuff, (short) (index + 2), mContext.mContextBuffer,
                        RANDOM_DATA1_OFFSET, BlOCK_16BYTES);
        ...
        // set SCP status
        mContext.mContextBuffer[SCP_STATUS_OFFSET] = SC1_SELECT_ADF;
        return (short) (generateSelectADFResponse(dataBuff, dataOffset, cDataBuffLength,
                tagOrSelKey, oidMatch ? mContext.mContextBuffer : null, SELECTED_OID_OFFSET,
                mContext.mContextBuffer[SELECTED_OID_LENGTH_OFFSET]));
    }

    private short generateFCIdeviceIdentity(byte[] outBuff, short outBuffOffset, short outBuffLength) {
        ...
        short offset = outBuffOffset;
        {
            ...
            {
                offset = mBerTlvBuilder.addTlv(outBuff, offset, outBuffLength,
                                               FCI_84, mContext.mContextBuffer, SELECTED_OID_OFFSET,
                                               (short) mContext.mContextBuffer[SELECTED_OID_LENGTH_OFFSET]);

                offset = mBerTlvBuilder.addTlv(outBuff, offset, outBuffLength,
                                               FCI_86, mContext.mContextBuffer, DEVICE_IDENTIFIER_OFFSET,
    ...
    private short generateFCIephemeralKey(byte[] outBuff, short outBuffOffset, short outBuffLength, boolean dummyKey) {
        ...
        short offset = outBuffOffset;
        {
            ...
            mBerTlvBuilder.startCOTag(offset);
            {
                ...
                offset = mBerTlvBuilder.addTlv(outBuff, offset, outBuffLength,
                                               ...
                                               pubKeyLen);
                if (!dummyKey) {
                    Util.arrayCopyNonAtomic(mOutData, (short) 0, mContext.mContextBuffer,
                            EPHEMERAL_PUBKEY2_OFFSET, pubKeyLen);
                    ((ECPrivateKey) mEcKeyPair.getPrivate()).getS(mContext.mContextBuffer,
                            EPHEMERAL_PRIKEY2_OFFSET);
                }
            }
            offset = mBerTlvBuilder.endCOTag(outBuff, offset, FCI_6F);
        }
    ...
    private void generateCryptogram(byte[] keyBuff, short encKeyBuffOffset) {
        ...
        mInData[inDataLength++] = mContext.mContextBuffer[SELECTION_INDEX_OFFSET];
        Util.arrayCopyNonAtomic(mContext.mContextBuffer, DEVICE_IDENTIFIER_OFFSET, mInData, (short) 2,
                                (short) DEVICE_IDENTIFIER_SIZE);
        ...
        Util.arrayCopyNonAtomic(mOutData, (short) 0, mContext.mContextBuffer, RANDOM_DATA0_OFFSET,
                BlOCK_16BYTES);
        ...
        if (mCrypto.genAes128CbcNopadOutput(Cipher.MODE_ENCRYPT, keyBuff, encKeyBuffOffset,
                mContext.mContextBuffer, RANDOM_DATA0_OFFSET, BlOCK_16BYTES, mInData, (short) 0,
    ...
    private short responseSelectADFSC2(byte[] dataBuff, short cDataBuffOffset, short cDataBuffLength,
            byte p1, byte p2, boolean oidMatch) {
        // (Table:37 CSML_r159)
        // Table 64 - ADF Extended Options
        boolean adfPrivacy = ClientContext.getADFPrivacy(mContext.mContextBuffer, SELECTED_OID_OFFSET,
                mContext.mContextBuffer[SELECTED_OID_LENGTH_OFFSET]);

        if (p1 == C_04) {
            if (oidMatch && !adfPrivacy) {
                // send FCI
                return (short) (generateFCIdeviceIdentity(dataBuff, cDataBuffOffset, cDataBuffLength)
                        - cDataBuffOffset);
            } else if (p2 == C_00) {
            ...
            if (oidMatch && adfPrivacy) {
                ...
                short encKeyOffset = ClientContext.getKeyOffet(SC2_PRIVACY_KEYSET, ENC_KEYTYPE, mInData,
                        IN_DATA_KEYSET_OFFSET, keySetLen);
                mContext.mContextBuffer[P2_OFFSET] = p2;
        ...
        return (short) (generateFCIephemeralKey(dataBuff, cDataBuffOffset, cDataBuffLength, !oidMatch)
                - cDataBuffOffset);
    }

    //
    // One alternative would be for a FiRa Device to select the ADF using symmetric keys first and if unsuccessful
    // try using asymmetric keys. To avoid these two successive selections, Secure Channel 2 provides a
    // seamless authentication method switch feature where an ADF selected with asymmetric keys may respond
    // using symmetric keys provided the following conditions are met:
    ...
    // authentication, a FiRa Device 1 has updated the ADF of FiRa Device 2 with symmetric keys at a
    // previous point in time).
    private short parseSelectAdfSC2(byte[] dataBuff, short dataOffset, short dataLength, short cDataOffset) {
        ...
        switch (authMethod) {
            ...
            case ASYM_MUTUAL_SEAMLESS:
                mContext.mContextBuffer[AUTHENTICATE_METHOD_OFFSET] = authMethod;
                break;
            default:
                ISOException.throwIt(ISO7816.SW_DATA_INVALID);
        }
        ...
        short rDataLength = 0;
        boolean oidMatch = false;
        mContext.mContextBuffer[SELECTION_INDEX_OFFSET] = 0;
        ...
        if (p1 == C_04) {
            ...
            Util.arrayCopyNonAtomic(dataBuff, (short) (cDataBuffOffset + 5),
                    mContext.mContextBuffer, EPHEMERAL_PUBKEY1_OFFSET, EC_PK_KEY_LENGTH);
        ...
        while (index < rDataLength) {
            ...
            index += (2 + dataBuff[(short) (index + 1)]);
            mContext.mContextBuffer[SELECTION_INDEX_OFFSET]++;
        }

        // set SCP status
        mContext.mContextBuffer[SCP_STATUS_OFFSET] = authMethod == SYM ? SC2_SELECT_ADF_SYS
                : SC2_SELECT_ADF_ASYS;
    ...
    // Public functions
    public short parseSelectAdf(byte[] dataBuff, short dataOffset, short dataLength, short cDataOffset) {
        ...
        } else if (dataBuff[cDataBuffOffset] == T_80 && dataBuff[(short) (cDataBuffOffset + 1)] == 0x01) {
    ...
    public short processGeneralAuthentication(byte[] dataBuff, short dataBuffOffset,
        ...
        short msg7CByteCnt = BerTlvParser.getTotalLengthBytesCount(dataBuff, (short) (cDataBuffOffset + 1));

        if ((dataBuff[(short) (cDataBuffOffset + msg7CByteCnt + 1)] == T_81 &&
                dataBuff[(short) (cDataBuffOffset + msg7CByteCnt + 2)] == C_00) || 
                (dataBuff[(short) (cDataBuffOffset + msg7CByteCnt + 1)] == T_82 &&
                 dataBuff[ISO7816.OFFSET_P1] == (byte) 0x00)) {

            return processSymmetricSC1Authentication(dataBuff, dataBuffOffset, dataBuffLength, cDataOffset,
                    msg7CByteCnt);

        } else if((dataBuff[(short) (cDataBuffOffset + msg7CByteCnt + 1)] == T_81 ||
                        dataBuff[(short) (cDataBuffOffset + msg7CByteCnt + 1)] == T_82)) {
            return processSymmetricSC2Authentication(dataBuff, dataBuffOffset, dataBuffLength, cDataOffset,
                    adfPrivacy);
        } else if (dataBuff[(short) (cDataBuffOffset + msg7CByteCnt + 1)] == T_86) {
            return processAsymmetricSC2Authentication(dataBuff, dataBuffOffset, dataBuffLength, cDataOffset,
                    msg7CByteCnt);
        }
