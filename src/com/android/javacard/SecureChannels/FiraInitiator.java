...
public class FiraInitiator {
    ...
    public FiraInitiator(FiraContext context, FiraClientContext firaClientContext, Crypto crypto, KeyPair ecKeyPair,
            Scp3Lib scp3Lib, FiraCommon common, byte[] inData, byte[] outData) {
        mContext = context;
    ...
    private short getGA1CmdSC1(byte[] buffer, short bufferOffset, short secureMsgAuthKey) {
        ...
        boolean tagNumberPresent = secureMsgAuthKey == 0 ? false : (secureMsgAuthKey > 31 ? true : false);

        // CLA/INS/P1P2
        buffer[index++] = C_00; // (or 0x0C when used during an ongoing Secure Messaging session)
        buffer[index++] = INS_GA1_GA2;
        buffer[index++] = C_00;
        mContext.mContextBuffer[P2_OFFSET] = buffer[index++] = tagNumberPresent ? C_00 :
        ...
        if (tagNumberPresent) {
            ...
            Util.setShort(mContext.mContextBuffer, SC1_TAGNUMBER_OFFSET, secureMsgAuthKey);
            
        } else {
            Util.setShort(mContext.mContextBuffer, SC1_TAGNUMBER_OFFSET, (short) 0);
        }
    ...
    private short getGA2CmdSC1(byte[] buffer, short bufferOffset) {
        ...
        buffer[index++] = INS_GA1_GA2;
        buffer[index++] = C_00;
        buffer[index++] = mContext.mContextBuffer[P2_OFFSET];
        ...
        short outLen = mCrypto.genAes128CbcNopadOutput(Cipher.MODE_ENCRYPT, mInData, macKeyOffset,
                ...
                RANDOM_ICC_OFFSET, SIGNATURE_BLOCK_SIZE, mOutData, (short) 0);

        RandomData.getInstance(RandomData.ALG_FAST).nextBytes(mContext.mContextBuffer, RANDOM_IFD_OFFSET, BlOCK_16BYTES);
        RandomData.getInstance(RandomData.ALG_FAST).nextBytes(mContext.mContextBuffer, KIFD_OFFSET, BlOCK_16BYTES);
        Util.arrayCopyNonAtomic(mContext.mContextBuffer, RANDOM_IFD_OFFSET, mInData, (short) 0, BlOCK_16BYTES);
        Util.arrayCopyNonAtomic(mContext.mContextBuffer, RANDOM_ICC_OFFSET, mInData, BlOCK_16BYTES, BlOCK_16BYTES);
        Util.arrayCopyNonAtomic(mContext.mContextBuffer, KIFD_OFFSET, mInData, (short) (BlOCK_16BYTES * 2), BlOCK_16BYTES);
        // NOTE:- Check text2 in future
        short inLen = Crypto.addPaddingM2(mInData, (short) 0, (short) (BlOCK_16BYTES * 3));

        // E.IFD
        short eIFDLen = mCrypto.genAes128CbcNopadOutput(Cipher.MODE_ENCRYPT, mInData, encKeyOffset, mOutData,
                (short) 0, outLen, mInData, (short) 0, inLen, mInData, inLen);

        // M.IFD
        short mIFDLen = mCrypto.genCmacAes128(mInData, macKeyOffset, SIGNATURE_BLOCK_SIZE, mInData, inLen,
                eIFDLen, mInData, (short) (inLen + eIFDLen));
        ...
        index += BerTlvBuilder.fillLength(buffer,
                (short) (cryptoIFDLen + 1 + (cryptoIFDLen < 0x80 ? 1 : (cryptoIFDLen < 0x100 ? 2 : 3))), index);
    ...
    private short getGA1SymmetricCmdSC2(byte[] buffer, short bufferOffset, byte kvn,
        ...
        short index = bufferOffset;
        boolean privacy = ClientContext.getADFPrivacy(mContext.mContextBuffer, SELECTED_OID_OFFSET,
        ...
        RandomData.getInstance(RandomData.ALG_FAST).nextBytes(mContext.mContextBuffer, CHALLENGE1_OFFSET,
                BlOCK_16BYTES);
        index = Util.arrayCopyNonAtomic(mContext.mContextBuffer, CHALLENGE1_OFFSET, buffer, index, BlOCK_16BYTES);
        ...
        if (privacy) {
            ...
            mInData[16] = 0x00;
            Util.arrayCopyNonAtomic(mContext.mContextBuffer, DEVICE_IDENTIFIER_OFFSET,
            ...
            short macKeyOffset = ClientContext.getKeyOffet(SC2_PRIVACY_KEYSET, MAC_KEYTYPE,
                    mInData, IN_DATA_KEYSET_OFFSET, keySetLen);
            length = mCrypto.genCmacAes128(mInData, macKeyOffset, SIGNATURE_BLOCK_SIZE, mInData, (short) 0, length,
                    mOutData, (short) 0);
    ...
    private short getGA2SymmetricCmdSC2(byte[] buffer, short bufferOffset, byte securityLevel, byte kvn) {
        ...
        Util.arrayCopyNonAtomic(mContext.mContextBuffer, CHALLENGE1_OFFSET,
                mInData, (short) 0, BlOCK_16BYTES);
        Util.arrayCopyNonAtomic(mContext.mContextBuffer, CHALLENGE2_OFFSET,
        ...
        index = Util.arrayCopyNonAtomic(mOutData, (short) (SIGNATURE_BLOCK_SIZE * 3), buffer, index, len);
        buffer[index++] = T_84;
        buffer[index++] = (byte) 0x10;
    ...
    private short getGAAsymmetricCmdSC2(byte[] buffer, short bufferOffset, byte securityLevel, byte kvn) {
        ...
        short ShSSize = mCrypto.generateSecretEC_SVDP_DHC(mContext.mContextBuffer, EPHEMERAL_PRIKEY1_OFFSET,
                EC_SK_KEY_LENGTH, mContext.mContextBuffer, EPHEMERAL_PUBKEY2_OFFSET, EC_PK_KEY_LENGTH,
                mInData, (short) 0);
        ...
        // TODO: Key
        short kdkLength = mCrypto.genCmacAes128(mContext.mContextBuffer, KEY_PRI_ENC_OFFSET,
                EC_SK_KEY_LENGTH, mInData, (short) 0, (short) (ShSSize + BlOCK_16BYTES), mOutData, (short) 0);
        ...
        // save K_SesAuthEnc
        Util.arrayCopyNonAtomic(mOutData, kdkLength, mContext.mContextBuffer, KSES_AUTHENC_OFFSET, kSesAuthEncLength);
        ...
        short keySetLen = mFiraClientContext.getKeySet(mContext.mContextBuffer[P2_OFFSET],
                mInData, IN_DATA_KEYSET_OFFSET);
        // short encKeyOffset = ((AdfStore) mFiraClientContext[0]).getKeyOffet(SC2_KEYSET, PRIVATE_KEYTYPE, mInData, IN_DATA_KEYSET_OFFSET, keySetLen);
        short mesgLen = mCrypto.ecdSAPlainSignatureSha256(mContext.mContextBuffer, EC_KEY_PRIV1_OFFSET,
        ...
        short mesgEncLen = mCrypto.genAes128CbcNopadOutput(Cipher.MODE_ENCRYPT, mOutData, kdkLength, mScp3Lib.mNullBytes16,
                (short) 0, BlOCK_16BYTES, mInData, mesg1ExtLength, mesgLen, mInData, (short) (mesg1ExtLength + mesgLen));

        //////////////////////////////////////////////
        // now derive keys
        // delaying scp03rootkey calculation
        short kScp03RootLength = mCrypto.cmacKdfCounterModeFiRa2(mOutData, (short) 0, T_4B, T_B4, mOutData,
        ...
        Util.arrayFillNonAtomic(mInData, (short) contextIndex, (short) (SIGNATURE_BLOCK_SIZE * 2), C_00);
    ...
    public short getGA(byte[] buffer, short bufferOffset, short kvn, short kid, byte securityLevel) {
        ...
        if (mContext.mContextBuffer[SCP_STATUS_OFFSET] == SC2_SELECT_ADF_ASYS) {
        ...
        } else if (mContext.mContextBuffer[SCP_STATUS_OFFSET] == SC2_SELECT_ADF_SYS) {
        ...
        } else if (mContext.mContextBuffer[SCP_STATUS_OFFSET] == SC1_SELECT_ADF) {
            mContext.mContextBuffer[SCP_STATUS_OFFSET] = SC1_GA1;
            return getGA1CmdSC1(buffer, bufferOffset, kvn /*SC kvn*/);
        } else if (mContext.mContextBuffer[SCP_STATUS_OFFSET] == SC2_GA1) {
        ...
        } else if (mContext.mContextBuffer[SCP_STATUS_OFFSET] == SC1_GA1) {
    ...
    public short getSelectADFCmdSC1(byte[] buffer, short bufferOffset, byte[] oidData,
        ...
        // The extended length APDU fields shall be supported.
        buffer[index++] = 0x00;
        index = Util.setShort(buffer, index, (short) (oidDataLength + 21)); // 21 is subsequent pre-calculated length
        ...
        return (short) (Util.arrayCopyNonAtomic(mContext.mContextBuffer, RANDOM_DATA1_OFFSET,
    ...
    public short getSelectADFCmdSC2(byte[] buffer, short bufferOffset, byte[] oidData,
        ...
        short index = bufferOffset;
        short dataLen = (authMethod != SYM) ? (short) (oidDataLength + 6) : (short) (oidDataLength + 3);

        // CLA/INS/P1P2
        buffer[index++] = T_80;
        buffer[index++] = INS_SELECT_ADF;
        buffer[index++] = (authMethod != SYM) ? C_00 : C_04;
        mContext.mContextBuffer[P2_OFFSET] = buffer[index++] = keyRef;
        ...
        if (authMethod != SYM) {
            ...
            ((ECPublicKey) mEcKeyPair.getPublic()).getW(mContext.mContextBuffer, EPHEMERAL_PUBKEY1_OFFSET);
            ((ECPrivateKey) mEcKeyPair.getPrivate()).getS(mContext.mContextBuffer, EPHEMERAL_PRIKEY1_OFFSET);
            buffer[index++] = (byte) 0x81;
            buffer[index++] = EC_PK_KEY_LENGTH;
            index = Util.arrayCopyNonAtomic(mContext.mContextBuffer, EPHEMERAL_PUBKEY1_OFFSET, buffer,
                    index, EC_PK_KEY_LENGTH);
            mContext.mContextBuffer[SCP_STATUS_OFFSET] = SC2_SELECT_ADF_ASYS;
        } else {
            mContext.mContextBuffer[SCP_STATUS_OFFSET] = SC2_SELECT_ADF_SYS;
        }
