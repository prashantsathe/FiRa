...
public class FiraCommon {
    ...
    public void generateDefaultUWBKeys(byte[] keyBuff, short keyBuffOffset, byte[] context, short contextOffset,
            short contextLength, byte deviationConstant, boolean uwbSessionKeyFlag) {
        // generate and set UWB default key and session ID
        // The default values for both UWB Session Key and UWB Session ID shall be calculated according to
        // GPC_2.3_D_SCP03_v1.2:April 2020 [17] section 4.1.5, using the S-MAC as root key.
        // "As mUWBsessionID is 4 bytes storing mUWBsessionID in mUWBsessionKey temporarily"
        if (mScp3Lib.scp03KDF(keyBuff, keyBuffOffset, context, contextOffset,
                contextLength, deviationConstant, uwbSessionKeyFlag, mContext.mContextBuffer,
                UWB_SESSIONKEY_OFFSET) != SIGNATURE_BLOCK_SIZE ||
            Util.arrayCopyNonAtomic(mContext.mContextBuffer, UWB_SESSIONKEY_OFFSET, mContext.mContextBuffer,
                UWB_SESSIONID_OFFSET, UWB_SESSION_ID_SIZE) != (short) (UWB_SESSION_ID_SIZE + UWB_SESSIONID_OFFSET) ||
    ...
    public void setScpAndUWBsessionKey(byte[] preText, short preTextOffset, byte[] kIfdIccBuffer,
            ...
            short macKeyOffset, byte[] outData, short outDataOffset) { // zero

        preText[(short) (preTextOffset + 11)] = (byte) 0x04;
        short keyLength = mCrypto.cmacKdfCounterMode(keyBuff, encKeyOffset, preText, preTextOffset, (short) 12,
                preText, (short) (preTextOffset + 12), (short) 3, (byte) 0x01,
                kIfdIccBuffer, kIfdIccBufferOffset, kIfdIccBufferLength, outData,
                outDataOffset);

        preText[(short) (preTextOffset + 11)] = (byte) 0x06;
        keyLength = mCrypto.cmacKdfCounterMode(keyBuff, macKeyOffset, preText, preTextOffset, (short) 12,
                preText, (short) (preTextOffset + 12), (short) 3, (byte) 0x01,
                kIfdIccBuffer, kIfdIccBufferOffset, kIfdIccBufferLength, outData,
                (short) (keyLength + outDataOffset));

        // NOTE: Here we use scp03 class for secure channel data wrapping where RMAC and SMAC are same
        //       and we are setting secure channel level as CDECMAC_RENCMAC
        ...
        // generate and set UWB default key and session ID
        generateDefaultUWBKeys(outData, (short) (keyLength + outDataOffset), kIfdIccBuffer, kIfdIccBufferOffset,
                kIfdIccBufferLength, DERIVATION_UWB_SESSION_ID, true);
    }

    public void deriveKeys(byte[] encKey, short encKeyOffset, byte[] macKey, short macKeyOffset,
            byte[] context, short contextOffset, short contextlength, byte[] output, short outputOffset) {
        ...
        if (mScp3Lib.scp03KDF(encKey, encKeyOffset, context, contextOffset,
            ...
            mScp3Lib.scp03KDF(macKey, macKeyOffset, context, contextOffset,
                    contextlength, DERIVATION_SMAC, false, output, (short) (outputOffset + SIGNATURE_BLOCK_SIZE))
                != SIGNATURE_BLOCK_SIZE ||
            mScp3Lib.scp03KDF(macKey, macKeyOffset, context, contextOffset,
                    contextlength, DERIVATION_RMAC, false, output, (short) (outputOffset + (SIGNATURE_BLOCK_SIZE * 2)))
                != SIGNATURE_BLOCK_SIZE) {
            // Crypto error
            ISOException.throwIt(ISO7816.SW_UNKNOWN);
        }
    ...
    public void setSecurityLevel(byte level) {
        ...
        if (level == CDECMAC_RENCMAC || level == CDECMAC_RMAC || level == CMAC_RMAC ||
            ...
            mContext.mContextBuffer[SECURITY_LEVEL_OFFSET] = level;
        } else {
