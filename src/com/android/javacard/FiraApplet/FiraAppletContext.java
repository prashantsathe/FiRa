...
public class FiraAppletContext extends FiraClientContext {
  ...
  // returns null.This method is implemented based on an understanding that just like initiator the
  ...
  public short getSelectedKvn(byte kvnType, byte[] buf, short index) {
    byte[] mem = FiraRepository.getSlotSpecificAdfData(FiraSpecs.TAG_FIRA_SC_CRED, (byte) getSlot(),
        retValues);
    ...
    switch (kvnType) {
      ...
      case SC_KEY_SET:
        tagEnd = FiraUtil.getTag(FiraSpecs.TAG_FIRA_SC_SYMMETRIC_KEY_SET, mem, start, len, true,
            retValues);
        if (tagEnd == FiraSpecs.INVALID_VALUE) {
          tagEnd = FiraUtil.getTag(FiraSpecs.TAG_FIRA_SC_ASYMMETRIC_KEY_SET, mem, start, len, true,
              retValues);
        }
        break;
      case BASE_KEY_SET:
        tagEnd = FiraUtil.getTag(FiraSpecs.TAG_FIRA_SC_SYMMETRIC_BASE_KEY, mem, start, len, true,
            retValues);
        break;
      case UWB_ROOT_KEY_SET:
        tagEnd = FiraUtil.getTag(FiraSpecs.TAG_FIRA_SC_UWB_RANGING_ROOT_KEY, mem, start, len, true,
            retValues);
        break;
  ...
  //  TODO Currently we do not support ADF Provisioning scenario used by Manage ADF. This will be
  //   supported once the use-case becomes clear.
  public short getCAPublicKey(byte kvn, byte[] buf, short index) {
    ...
    if (getSlot() == FiraRepository.APPLET_SLOT ||
      ...
      byte[] mem = FiraRepository.getSlotSpecificAdfData(FiraSpecs.TAG_STORED_ADF_PROVISIONING_CRED,
          (byte) getSlot(), retValues);
      // First read the cred record which may have more than one key sets
      short end = FiraUtil.getTag(FiraSpecs.TAG_STORED_ADF_PROVISIONING_CRED, mem, retValues[1],
      ...
      short start = retValues[3];
      short len = retValues[2];
      ret = getPublicKey(mem, start, len,
          FiraSpecs.TAG_ADF_PROV_ASYMMETRIC_KEY_SET, FiraSpecs.TAG_ADF_PROV_SEC_CH_KVN, kvn,
          FiraSpecs.TAG_ADF_PROV_CA_PUB_KEY, buf, index);
      if (ret > 0) {
        end = FiraUtil.getTag(FiraSpecs.TAG_ADF_PROV_SEC_CH_ID, mem, start, len, true, retValues);
  ...
  private short getPublicKey(byte[] mem, short memStart, short memLen,
      short keySetTag, short keySetKvnTag, byte kvn, short credTag, byte[] buf, short start) {
    // Save kvn
    buf[start] = kvn;
    // Search for key set with given kvn
    short end = FiraUtil.search(keySetTag, keySetKvnTag, buf, start, (short) 1,
        mem, memStart, memLen, retValues);
  ...
  public short getSDCertificate(byte[] buf, short index) {
    ...
    short end = FiraUtil.getTag(FiraSpecs.TAG_CERT, mem, retValues[1], retValues[2],
        true, retValues);
