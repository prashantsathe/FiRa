...
public class FiraUtil {
    ...
    /**
     * Read Tag id from the BER TLV object. In FiraApplet the tag id will only be
     * one or two bytes long. if the tag id is more than 2 bytes then
     * ISO7816.SW_WRONG_DATA exception is thrown.
     *
     * @param buf       - buffer of data
     * @param index     - start index
     * @param len       - length of the buffer
    ...
    public static short getNextDGITag(byte[] buf, short index, short len, boolean skip,
            short[] retValues) {
        short end = (short) (index + len);
        ...
        retValues[0] = index;
        // DGI tags are always 2 bytes
        retValues[1] = Util.getShort(buf, index);
        index += 2;

        // DGI lengths are either 1 or 3 bytes long
        if ((buf[index] != (byte) 0xFF)) {
            retValues[2] = buf[index];
        } else {
            index++;
            retValues[2] = Util.getShort(buf, index);
            index++;
        }
        index++;
        retValues[3] = index;
    ...
    /**
     * Read Tag length from the BER TLV object. In FiraApplet the tag length will at
     * maximum will be 2 bytes long i.e. 32K. if the tag length is more than 2 bytes
    ...
    /**
     * Read the BER TLV object pointed by index. If TLV object is larger than the
     * len of the buffer then ISO7816.SW_WRONG_LENGTH exception is thrown.
     *
     * @param buf       - buffer of data
     * @param index     - start index
     * @param len       - length of the buffer
     * @param skip      - if true then skip the receding 0xFFs or 0s
     * @param retValues The returned data of the TLV object. - retValues[0] - start
     *                  index of TLV object - retValues[1] - tag id. - retValues[2]
     *                  - tag length - retValues[3] = start index of TLV value
     *                  field.
     * @return index pointing at start of the next byte following the end of TLV
     *         object.
     */
    public static short getNextTag(byte[] buf, short index, short len, boolean skip,
            short[] retValues) {
        if (len == 0) {
            return FiraSpecs.INVALID_VALUE;
        }
    ...
    /**
     * Read the BER TLV object pointed by index and matching the tag in the buffer.
     * If TLV object is larger than the len of the buffer then
     * ISO7816.SW_WRONG_LENGTH exception is thrown.
     *
     ...
     *                  - tag length - retValues[3] = start index of TLV value
     *                  field.
     * @return index pointing at start of the next byte following the end of the
     *         desired TLV object. If the tag is not present in the buffer than the
     *         FIRASpecs.INVALID_VALUE is returned
     */
    ...
    public static short pushBERTlv(byte[] buf, short index, short tag, byte[] data, short start,
            short len) {
    ...
    public static short push(byte[] buf, short index, short len, byte[] mem, short memIndex,
            short memLen, short[] retValues, short end) {
        // Read the next tag
    ...
    public static short pushTag(byte[] buf, short index, short len, byte[] mem, short memIndex,
            short memLen, short[] retValues, short end) {
        // Read the next tag
        short tagEnd = getNextTag(buf, index, len, true, retValues);
        index = retValues[3];
    ...
    public static short search(short parentTag, short tag, byte[] val, short valStart, short valLen,
        ...
        // Read DOs one by one - index points to beginning of the and the end points to
        // end of all the parent DOs.
