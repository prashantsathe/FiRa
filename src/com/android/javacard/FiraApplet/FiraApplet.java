...
public class FiraApplet extends Applet implements AppletEvent, MultiSelectable, ExtendedLength {
  ...
  public void process(APDU apdu) throws ISOException {
    ...
    //if (buf[ISO7816.OFFSET_LC] != 0 || apdu.getBuffer().length < IMPL_APDU_BUFFER_MAX_SIZE) {
    //  ISOException.throwIt(ISO7816.SW_CONDITIONS_NOT_SATISFIED);
    //}
    ...
    // TODO: confirm with chirag and remove the following
    // If the context is secure then unwrap
//    if (context.isLocalSecure() || context.isRemoteSecure()) {
//      len = unwrap(buf, index, len, context);
//    }
...
//    case FIRASpecs.INS_STORE_DATA:
      ...
      case FiraSpecs.INS_GET_DATA:
        len =
            processGetDataCmd(
                (short)(buf[ISO7816.OFFSET_P1] << 8 | (buf[ISO7816.OFFSET_P2] & 0x00FF)),
                buf, index, len, context, retValues);
        break;
  ...
  private void assertP1P2(byte instruction, short p1, short p2) {
    ...
    switch (instruction) {
      case FiraSpecs.INS_SWAP_ADF:
        ...
        break;
      case FiraSpecs.INS_PROVISION_SERVICE_APPLET:
        err = p1 != P1_ADD_SERVICE_APPLET &&
            p1 != P1_REMOVE_SERVICE_APPLET || p2 != 0;
        break;
      case FiraSpecs.INS_INITIATE_TRANSACTION:
      ...
      case FiraSpecs.INS_GET_DATA:
        ...
        err = p1p2 != FiraSpecs.TAG_PA_LIST &&
            p1p2 != FiraSpecs.TAG_APPLET_CERT_STORE &&
            p1p2 != FiraSpecs.TAG_STATIC_STS_SLOT_OID &&
            p1p2 != 0x3FFF; // This is used for arbitrary GET Data command.
        break;
      case FiraSpecs.INS_PUT_DATA:
      case FiraSpecs.INS_CREATE_ADF:
      case FiraSpecs.INS_DELETE_ADF:
...
//    case FIRASpecs.INS_STORE_DATA:
  ...
  // TODO For JCOP this should just return APDU.getCLAChannel()
  ...
  private short processPutDataCmd(byte[] buf, short inputStart, short inputLen,
    ...
    // Either local or remotely secure instruction. if remotely secured then slot should be
    // valid, and it should not be root slot. If locally secured then slot should be valid and
    // not APPLET SLOT.
    short slot = context.getSlot();
    if ((context.isRemoteUnSecure() && context.isLocalUnSecure()) ||
        (context.isRemoteSecure() &&
            (slot == FiraSpecs.INVALID_VALUE ||
                slot == FiraRepository.ROOT_SLOT ||
                slot == FiraRepository.APPLET_SLOT)) ||
        (context.isLocalSecure() &&
            slot == FiraRepository.APPLET_SLOT)) {
      ISOException.throwIt(ISO7816.SW_CONDITIONS_NOT_SATISFIED);
    }
  ...
  public short processTunnelCmd(byte[] buf, short inputStart, short inputLen,
    ...
    assertOrderedStructure(buf, inputStart, inputLen, FiraSpecs.DATA_TUNNEL,
        true, buf, (short) (IMPL_APDU_BUFFER_MAX_SIZE - FiraSpecs.IMPL_SCRATCH_PAD_MAX_SIZE));
    ...
    // Note: if instruction gets or sets session data and or terminates session then set the context
    // accordingly so that rds can be generated and/or session can be terminated at the completion
    // of the command. This is done in dispatch response handling in dispatchSecure method.
    if (ins == FiraSpecs.INS_PUT_DATA) {
      assertOrderedStructure(buf, insStart, insLen, FiraSpecs.DATA_REMOTE_PUT_DATA,
          true, buf, (short) (IMPL_APDU_BUFFER_MAX_SIZE - FiraSpecs.IMPL_SCRATCH_PAD_MAX_SIZE));
    ...
    // wrap the entire instruction include apdu header - this is wrapping the tunneled command.
  ...
  private short dispatchUnsecure(byte[] buf, short index, short len, FiraAppletContext context,
    ...
    return pushDispatchResponse(buf, index, len, status, (byte) eventId, buf, (short) (index + len),
        eventDataLen, retValues);
  }
  ...
  private short pushDispatchResponse(byte[] buf, short index, short len, short status, byte eventId,
    ...
    // Note: There can be upto 8 tags in a response and if we consider 4 bytes per tag then
  ...
  // The command supports is mainly TUNNEL command. Initiator side will originate the command in
  // processTunnel. So on initiator this method is called to handle the response of tunnel
  ...
  private short dispatchSecure(byte[] buf, short index, short len,
    ...
    if (status == FiraSpecs.VAL_PROP_DISPATCH_RESP_STATUS_RET_PEER) {
      // retValues[1] (lastTag) represent the tag value read inside 'handleTunneledCommand'
      // if last tag is FiraSpecs.TAG_TERMINATE_SESSION then avoid any channel operation
      if (lastTag != FiraSpecs.TAG_TERMINATE_SESSION)
          len = wrap(buf, index, len, context);
    }

    index = pushDispatchResponse(buf, index, len, status,
  ...
  private short handleCustomGetCmd(byte[] buf, short inputStart, short inputLen,
    ...
    // There is a special case in which the FIRA Fw can just request to get Controlee Info from
    // Shared Adf part. In this case the slot will be the root slot.
    if (context.getSlot() == FiraRepository.ROOT_SLOT &&
        retValues[1] == FiraSpecs.TAG_UWB_CONTROLEE_INFO &&
        retValues[2] == 0) {
      ...
      inputLen = handleGetCommand(tag, buf, retValues[3], retValues[2], context,
          retValues);
  ...
  private short getPutCmdRouteInfo(byte[] buf, short index, short len, FiraAppletContext context) {
    ...
    if (routeTarget != FiraSpecs.VAL_SERVICE_APPLET) {
      // TODO Currently, routing to application is not supported, but can be easily supported in
      //  future.
      ISOException.throwIt(ISO7816.SW_INS_NOT_SUPPORTED);
    } else {
  ...
  private short sendRDS(byte[] buf, short index, short len) {
    SecureUwbService susApplet = (SecureUwbService) JCSystem.getAppletShareableInterfaceObject(
        new AID(FiraSpecs.SUS_APPLET_AID, (short) 0, (byte) FiraSpecs.SUS_APPLET_AID.length),
  ...
  private void generateAndSendRDS(byte[] buf, short index, short len, FiraAppletContext context) {
    ...
    short end = FiraUtil.getTag(FiraSpecs.TAG_UWB_SESSION_DATA,
        mem, retValues[1], retValues[2], true, retValues);
    if (end == FiraSpecs.INVALID_VALUE) {
      return;
    }

    // TAG_UWB_SESSION_DATA is a constructed tag so start next search in 'mem' with TLV value
    short valueLen = retValues[2];
    short valueIndex = retValues[3];

    end = FiraUtil.getTag(FiraSpecs.TAG_UWB_SESSION_ID, mem, retValues[3], valueLen,
        true, retValues);
    if (end == FiraSpecs.INVALID_VALUE) {
        return;
    }
  ...
  private short handleTunneledCommand(byte[] buf, short index, short len,
    ...
    if (sessionData) {
      generateAndSendRDS(buf, (short) (index + len), FiraSpecs.IMPL_MAX_RDS_DATA_SIZE, context);
  ...
  private short getAllControleeInfo(byte[] buf, short index, short len, FiraAppletContext context,
    ...
    short adfEnd = FiraUtil.getTag(FiraSpecs.TAG_UWB_CONTROLEE_INFO, adfMem, adfStart, adfLen, true,
        retValues);
    ...
    // Version can be present in shared data as well as adf data. Preference is given to shared data
  ...
  private short processImportADFCmd(byte[] buf, short inputStart, short inputLen,
    ...
    if (FiraUtil.getTag(FiraSpecs.TAG_OID, buf, inputStart, inputLen, true, retValues) ==
        FiraSpecs.INVALID_VALUE) {
      // Shift by APPLET_OID bytes
      short aopOidLen = (short) FiraSpecs.TAG_APPLET_OID.length;
      Util.arrayCopyNonAtomic(buf, inputStart, buf, (short) (inputStart + aopOidLen), inputLen);
      Util.arrayCopyNonAtomic(FiraSpecs.TAG_APPLET_OID, (short) 0, buf, inputStart, aopOidLen);
      inputLen += aopOidLen;
    }
    byte[] outBuf = buf;
      ...
      case STRUCTURE:
        if (ordered) {
          assertOrderedStructure(buf, index, len,
              FiraSpecs.expressionTable(tagIndex), false, scratchPadBuf,
              scratchPadIndex);
        } else {
          assertUnorderedStructure(buf, index, len,
              FiraSpecs.expressionTable(tagIndex), count, false, scratchPadBuf,
              scratchPadIndex);
        }
        index += len;
        break;
  ...
  // This method is different from rest of the process methods. This method treats buf as apdu
  // because it handles also P1 and P2. So care must be taken that complete apdu buf is passed to
  // this method.
  ...
  private short processDeleteAdfCmd(byte[] buf, short inputStart, short inputLen,
    ...
    if (slotId != FiraSpecs.INVALID_VALUE &&
        slotId != FiraRepository.ROOT_SLOT &&
  ...
  private short processManageADFCmd(byte[] buf, short inputStart, short inputLen,
    ...
    } else if (buf[ISO7816.OFFSET_P1]
        == FiraSpecs.INS_MANAGE_ADF_FINISH_P1) { // last manage adf and so commit the adf
  ...
  private short processCreateAdfCmd(byte[] buf, short inputStart, short inputLen,
      FiraAppletContext context, short[] retValues) {
    // Context should have root slot to begin with because create adf cannot be done in slot
    // specific way.
    if (context.getSlot() != FiraRepository.ROOT_SLOT) {
      ISOException.throwIt(ISO7816.SW_CONDITIONS_NOT_SATISFIED);
    }
    ...
    // This is done because ADF Provisioning Credentials tag and Fira SC Credentials tags are the
    // same in the Fira Specs. It is not clear whether this is intentional.
    short tagEnd = FiraUtil.getTag(FiraSpecs.TAG_ADF_PROVISIONING_CRED, buf, inputStart, inputLen,
        true, retValues);
    if (tagEnd != FiraSpecs.INVALID_VALUE) {
  ...
  private short processSelectAdf(byte[] buf, short inputStart, short inputLen,
      FiraAppletContext context, short[] retValues) {
    // There should not be already selected slot or if at all it should be root slot.
    if (context.getSlot() != FiraSpecs.INVALID_VALUE &&
        context.getSlot() != FiraRepository.ROOT_SLOT) {
      ISOException.throwIt(ISO7816.SW_CONDITIONS_NOT_SATISFIED);
    }
    // Select first OID in the list of OIDs.
    short randomDataEnd = FiraUtil.getTag(FiraSpecs.TAG_RANDOM_DATA_1_2, buf, inputStart, inputLen,
        true, retValues);
  ...
  private short makeSelectResponse(byte[] buf, short oidStart, short oidEnd, short randStart,
    ...
    short deviceUidEnd = FiraUtil.getTag(FiraSpecs.TAG_DEVICE_UID, mem, retValues[1], retValues[2],
        true, retValues);
  ...
  private short processProvisionSDCredentials(byte[] buf, short inputStart, short inputLength,
    ...
    if (tagEnd != FiraSpecs.INVALID_VALUE) {
      tagEnd = FiraUtil.getTag(FiraSpecs.TAG_DEVICE_UID, buf, inputStart, inputLength, true,
          retValues);
      if (tagEnd != FiraSpecs.INVALID_VALUE) {
        tagEnd = FiraUtil.getTag(FiraSpecs.TAG_APPLET_SECRET, buf, inputStart, inputLength, true,
            retValues);
      }
    }
    if (tagEnd == FiraSpecs.INVALID_VALUE) {
