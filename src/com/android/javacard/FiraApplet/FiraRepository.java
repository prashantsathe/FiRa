...
/**
 * The repository stores persistent and transient data. The persistent data consists of applet and
 * static and shared adf data. The data is stored as data objects identified by tags. The adf tags
 * are specified by FiraApplet. The memory is partitioned in various parts and every part specific
 * data segment has max length and used length. The slot specific data is divided into static and
 * dynamic parts. Also, the non slot specific data is divided into shared adf and applet specific
 * parts. Slot specific adf data begins after shared data which in turn begins after applet specific
 * data.
 */
public class FiraRepository {
  ...
  static final short IMPL_SLOT_STATIC_ADF_DATA_MAX_SIZE = (short) (
      (FiraSpecs.IMPL_STATIC_SLOT_MAX_COUNT * FiraSpecs.IMPL_ADF_STATIC_DATA_MAX_SIZE_PER_SLOT)
  );
  private static final short PERSISTENCE_MEM_SIZE = (short) (
      FiraSpecs.IMPL_APPLET_SPECIFIC_DATA_MAX_SIZE +
  ...
  private static final short TRANSIENT_MEM_SIZE = (short) (
      (FiraSpecs.IMPL_STATIC_SLOT_MAX_COUNT * FiraSpecs.IMPL_ADF_EPHEMERAL_DATA_MAX_SIZE_PER_SLOT) +
          (FiraSpecs.IMPL_DYNAMIC_SLOT_MAX_COUNT * (FiraSpecs.IMPL_ADF_STATIC_DATA_MAX_SIZE_PER_SLOT
  ...
  public static void init() {
    ...
    //NOTE: for upgrade if new slots are added then they have to be added at the end. The old data
    // must be backed up.
    slots = new short[(short) ((
        FiraSpecs.IMPL_DYNAMIC_SLOT_MAX_COUNT + FiraSpecs.IMPL_STATIC_SLOT_MAX_COUNT) * 2)];
  ...
  private static void initDummyAppletData() {
    byte[] buf = {(byte) 0, (byte) 14,
        (byte) FiraSpecs.TAG_MASTER_KEY, (byte) 6, (byte) 0x0A, (byte) 0x0A, (byte) 0x0A,
        (byte) 0x0A,
        (byte) 0x0A, (byte) 0x0A, (byte) FiraSpecs.TAG_DEVICE_UID, (byte) 4, (byte) 0, (byte) 1,
        (byte) 0,
        (byte) 1};
  ...
  private static void initSlots(byte staticSlots, byte dynamicSlots) {
    byte i = 0;
    short pCursor = (short) (
        FiraSpecs.IMPL_APPLET_SPECIFIC_DATA_MAX_SIZE + FiraSpecs.IMPL_SHARED_ADF_DATA_MAX_SIZE);
    short tCursor = 0;
    staticSlots *= 2;
    dynamicSlots *= 2;
  ...
  // This method is not a transaction. The caller must invoke this method in a transaction.
  ...
  public static short reserveDynamicSlot() {
    return reserveSlot(FiraSpecs.IMPL_STATIC_SLOT_MAX_COUNT,
        (byte) (FiraSpecs.IMPL_STATIC_SLOT_MAX_COUNT + FiraSpecs.IMPL_DYNAMIC_SLOT_MAX_COUNT),
        transientMem);
  }
  ...
  public static void freeSlot(byte slotId) {
    ...
    if ((byte) (mem[cursor] & FREE) == 0) {
      ...
      Util.arrayFillNonAtomic(mem, cursor, FiraSpecs.IMPL_ADF_EPHEMERAL_DATA_MAX_SIZE_PER_SLOT,
          (byte) 0);
    }
  }
  ...
  public static byte getSlotUsingOid(byte[] oid, short index, short len) {
    ...
    while (i < FiraSpecs.IMPL_STATIC_SLOT_MAX_COUNT) {
      ...
      if (usedLen > 0) {
        ...
        if (tagEnd != FiraSpecs.INVALID_VALUE &&
            Util.arrayCompare(oid, index, mem, tagStart, len)
                == 0) { // len can be less than total oid len
          return i;
        }
      }
      i++;
  ...
  public static boolean isSlotFree(byte slot) {
    byte[] mem = getSlotSpecificAdfData(FiraSpecs.TAG_OID, slot, retValues);
